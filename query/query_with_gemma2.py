...
special_tokens = {
    "snunlp/bigdata_gemma2_9b_dora": { # snunlp 모델로 변경.
    ...
    "snunlp/bigdata_exaone3_7.8b_fft": { # snunlp 모델로 변경.
        "system_start": "[|system|]",
        "user_start": "[|user|]",
...
def get_sub_queries(query: str, llm, model_name: str) -> list[str]:
    ...
    args:
        ...
        model_name: 모델 이름
    return:
        sub_queries: 하위 쿼리 리스트
    '''

    # 프롬프트 설정
    if model_name == "snunlp/bigdata_gemma2_9b_dora" or model_name == "recoilme/recoilme-gemma-2-9B-v0.4":
