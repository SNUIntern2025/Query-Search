...
def prompt_routing(subqueries: List[str], model):
    '''
    subquery와 model을 받아, LLM prompting을 통해 routing을 병렬로 실행하는 함수
    ...
    # model = AutoModelForCausalLM.from_pretrained(model_id,
                                                ...
                                                #  ) 
    pipe = pipeline(
        "text-generation", model=model, device_map = "auto", max_new_tokens=10 # accelerator
    )
